...
def josephus_task(num_people, kill_num):
    ...
    survivors = [people + 1 for people in range(num_people)]
    deque_survivor = deque(survivors)
    while len(deque_survivor) > 1:
        deque_survivor.rotate(-kill_num)
        deque_survivor.pop()

    survivor = deque_survivor.pop()
    return survivor
